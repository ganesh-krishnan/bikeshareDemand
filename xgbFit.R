xgb_code <- getModelInfo ("xgbTree", regex=FALSE)[[1]]

xgb_code$parameters <- data.frame (parameter = c("nrounds", 
                                                 "max_depth", 
                                                 "eta",
                                                 "min_child_weight",
                                                 "subsample",
...
xgb_code$grid <- function (x, y, len = NULL, search="grid")
{
  if (search == "grid") {
...
xgb_code$fit <- function (x, y, wts, param, lev, last, classProbs, ...) 
{
  if (is.factor(y)) {
    if (length(lev) == 2) {
...
xgb_code$loop <- function (grid) 
{
  loop <- ddply(grid, 
                c("eta", "max_depth", "min_child_weight", 
